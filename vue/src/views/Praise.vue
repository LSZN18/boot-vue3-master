...
<template>
  <div>
    ...
    <div style="margin: 10px 0">
      ...
      <el-table-column  label="点赞内容"><template #default="scope"><span v-if="scope.row.fid">{{ state.dynamicOptions.find(v => v.id === scope.row.fid) ? state.dynamicOptions.find(v => v.id === scope.row.fid).name : '' }}</span></template></el-table-column>
