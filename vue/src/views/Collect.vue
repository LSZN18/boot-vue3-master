...
<template>
  <div>
    ...
    <div style="margin: 10px 0">
      ...
      <el-table-column label="收藏内容"><template #default="scope"><span v-if="scope.row.dynamicId">{{ state.dynamicOptions.find(v => v.id === scope.row.dynamicId) ? state.dynamicOptions.find(v => v.id === scope.row.dynamicId).name : '' }}</span></template></el-table-column>
    ...
    <el-dialog v-model="dialogFormVisible" title="收藏信息" width="40%">
      <el-form ref="ruleFormRef" :rules="rules" :model="state.form" label-width="80px" style="padding: 0 20px" status-icon>
        <el-form-item prop="dynamicId" label="收藏内容">
